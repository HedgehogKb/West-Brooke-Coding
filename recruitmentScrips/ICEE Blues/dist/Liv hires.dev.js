...
function interact(t) {
  if (t.player.hasActiveQuest(85)) {
    if (!npcExists(t, "Meradith")) {
      var quest = getQuest(t, 85);
      ...
      t.player.removeQuest(85);
      t.npc.say("You were trying to hire Meradith, but she already exists. I removed that quest. If you were tyring to completed a different quest then interact again.");
    }
  } else if (t.player.hasActiveQuest(86)) {
    if (!npcExists(t, "Andrew")) {
      var quest = getQuest(t, 86);
      var max = quest.getObjectives(t.player)[0].getMaxProgress();
      var curProg = quest.getObjectives(t.player)[0].getProgress();
      quest.getObjectives(t.player)[0].setProgress(cashProgress(curProg, max, t));
    } else {
      t.player.removeQuest(86);
      t.npc.say("You were trying to hire Andrew, but he already exists. I removed that quest. If you were tyring to completed a different quest then interact again.");
    }
  } else if (t.player.hasActiveQuest(87)) {
    var quest = getQuest(t, 87);
    var max = quest.getObjectives(t.player)[0].getMaxProgress();
    var curProg = quest.getObjectives(t.player)[0].getProgress();
    quest.getObjectives(t.player)[0].setProgress(cashProgress(curProg, max, t));
  }
