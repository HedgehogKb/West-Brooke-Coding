...
function timer(t) {
  ...
  ndata.put("CanGetPay", canPayed.toString()); //t.npc.say(ndata.get("CanGetPay").toString());
}
  ...
  if (t.dialog.getId() == 363) {
    if (t.option.getSlot() == 3) {
      ...
      if (contains(t.player.getDisplayName(), canPayed) != -1) {
        ...
        var payOutAmount = Math.round(0.8666 * t.player.getFactionPoints(22) - 1100);
        ...
        if (!t.npc.getTimers().has(location)) {
          t.npc.getTimers().start(location, 12000, false);
        }

        t.npc.sayTo(t.player, "Hold your horses. You've been doing great work, but you need to wait a bit before I can pay you.");
      }
    }
  }
}